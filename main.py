...
def inicializar_base_de_datos():
    ...
    if not os.path.exists(RUTA_BD):
        ...
        print("🔨 Iniciando construcción automática de la base de datos portátil...")
        ...
        INSERT OR IGNORE INTO `tblusuario`
          (`id_usuario`,`id_rol`,`nombre`,`apellido`,`correo`,`telefono`,`contrasena`,`estado`) VALUES
        ...
        INSERT OR IGNORE INTO `tblcita`
          (`id_cita`,`fecha`,`hora_inicio`,`hora_fin`,`id_usuario`,`id_odontologo`,`id_sala`) VALUES
        ...
        INSERT OR IGNORE INTO `tblconsentimiento`
          (`id_consentimiento`,`id_usuario`,`version`,`firma`,`fecha`) VALUES
        ...
        (5,'Revisión general',  'Consulta y revisión odontológica general',                            15000.00);

        -- ── TABLA: relación odontólogo ↔ tipo de tratamiento ───────────────
        CREATE TABLE IF NOT EXISTS `tblodontologo_servicio` (
          `id_odontologo` INTEGER NOT NULL,
          `id_tipo`       INTEGER NOT NULL,
          PRIMARY KEY (`id_odontologo`, `id_tipo`),
        ...
        CREATE INDEX IF NOT EXISTS `idx_os_odontologo` ON `tblodontologo_servicio` (`id_odontologo`);
        CREATE INDEX IF NOT EXISTS `idx_os_tipo`       ON `tblodontologo_servicio` (`id_tipo`);

        -- Asignaciones iniciales
        -- id 3 = William Alton     → Limpieza dental, Ortodoncia, Revisión general
        -- id 4 = Michael Phillips  → Limpieza dental, Endodoncia, Revisión general
        -- id 5 = Alberto Fernandez → Limpieza dental, Cirugía oral, Revisión general
        INSERT OR IGNORE INTO `tblodontologo_servicio` (`id_odontologo`,`id_tipo`) VALUES
        (3,1),(3,2),(3,5),
        (4,1),(4,3),(4,5),
        (5,1),(5,4),(5,5);

        -- ── TABLA: relación odontólogo ↔ su propio consultorio (1 a 1) ─────
        ...
        INSERT OR IGNORE INTO `tbltratamiento`
          (`id_tratamiento`,`id_tipo`,`diagnostico`,`valor`,`id_odontologo`,`id_usuario`) VALUES
        ...
        (306,6,'Caso profundo: implante dental',                       2700000.00,5,108),
        ...
        (310,10,'Se realiza la extracción de una muela del juicio',    1600000.00,5,110);
        ...
        (401,301,'2026-02-05'),(402,302,'2026-02-12'),(403,303,'2026-02-20'),
        (404,304,'2026-03-03'),(405,305,'2026-03-10'),(406,306,'2026-03-18'),
        (407,307,'2026-03-23'),(408,308,'2026-04-02'),(409,309,'2026-04-10'),
        (410,310,'2026-04-18');
        ...
        INSERT OR IGNORE INTO `tblhistorialclinico`
          (`id_historial_clinico`,`id_tratamiento`,`id_cita`,`observaciones`,`fecha`) VALUES
        ...
        (1,401,'2026-02-05'),(2,402,'2026-02-12'),(3,403,'2026-02-20'),
        (4,404,'2026-03-03'),(5,405,'2026-03-10'),(6,406,'2026-03-18'),
        (7,407,'2026-03-23'),(8,408,'2026-04-02'),(9,409,'2026-04-10'),
        (10,410,'2026-04-18');
        ...
        INSERT OR IGNORE INTO `tblhistorialmodificaciones`
          (`id_modificacion`,`id_cita`,`fecha_modificacion`,`fecha_nueva`) VALUES
        ...
        INSERT OR IGNORE INTO `tblpago`
          (`id_pago`,`id_factura`,`id_metodo`,`pagado`,`fecha_pago`) VALUES
        (501,401,1,1,'2026-02-05'),(502,402,2,0,NULL),(503,403,1,0,NULL),
        (504,404,3,1,'2026-03-10'),(505,405,1,1,'2026-03-10'),(506,406,2,0,NULL),
        (507,407,1,0,NULL),(508,408,2,1,'2026-04-04'),(509,409,3,1,'2026-04-10'),
        (510,410,1,0,NULL);
        ...
        INSERT OR IGNORE INTO `tblregistroclinico`
          (`id_registro`,`id_historial_clinico`,`id_odontologo`,`fecha`,`notes`) VALUES
        ...
        try:
            ...
            conn.commit()
            conn.close()
            print("🎉 Base de datos 'clinident.db' autoconstruida con todas las tablas y registros.")
        except Exception as e:
            print(f"❌ Error al construir la base de datos: {e}")
    else:
        print(f"📁 Base de datos detectada en: {RUTA_BD}")


def migrar_precio_base():
    """Garantiza que tbltipotratamiento tenga precio_base y 'Revisión general'."""
    conn   = sqlite3.connect(RUTA_BD)
    cursor = conn.cursor()
...
def migrar_columna_tratamiento():
    """Garantiza que tblcita tenga 'tratamiento' y tblagenda tenga 'motivo_cancelacion'."""
    conn   = sqlite3.connect(RUTA_BD)
    cursor = conn.cursor()
...
def migrar_odontologo_servicio():
    """
    Crea tblodontologo_servicio si no existe.
    Las asignaciones base SOLO se siembran la primera vez que la tabla
    queda vacía (justo después de crearla). En arranques posteriores NO
    se reinserta nada, para no pisar las ediciones hechas desde el
    panel de administración (admin_panel.py).

    Para agregar/quitar servicios a un odontólogo, usa el panel admin,
    o manualmente:
      INSERT INTO tblodontologo_servicio (id_odontologo, id_tipo) VALUES (?, ?)
    """
    conn   = sqlite3.connect(RUTA_BD)
    cursor = conn.cursor()
    ...
    # Solo siembra los datos base si la tabla está vacía (primera vez real).
    # Así, cualquier edición posterior desde el panel admin se conserva
    # entre reinicios del servidor.
    cursor.execute("SELECT COUNT(*) FROM tblodontologo_servicio")
    total_filas = cursor.fetchone()[0]

    if total_filas == 0:
        # id 3 = William Alton     → Limpieza dental(1), Ortodoncia(2), Revisión general(5)
        # id 4 = Michael Phillips  → Limpieza dental(1), Endodoncia(3),  Revisión general(5)
        # id 5 = Alberto Fernandez → Limpieza dental(1), Cirugía oral(4), Revisión general(5)
        cursor.executemany(
            "INSERT INTO tblodontologo_servicio (id_odontologo, id_tipo) VALUES (?, ?)",
            [(3, 1), (3, 2), (3, 5),
             (4, 1), (4, 3), (4, 5),
        ...
        conn.commit()
        print("✅ Tabla tblodontologo_servicio sembrada con asignaciones base (primera vez).")
    else:
        print(f"📁 tblodontologo_servicio ya tiene {total_filas} asignaciones — no se modifica.")

    conn.close()


def migrar_odontologo_sala():
    """
    Crea tblodontologo_sala (relación 1 a 1: cada odontólogo tiene su propio
    consultorio). Para cada odontólogo activo que todavía no tenga sala
    asignada, crea automáticamente un nuevo registro en tblsala llamado
    "Consultorio N" y lo vincula. Segura para correr en cada arranque:
    solo actúa sobre odontólogos que aún no tienen sala asignada, así que
    no pisa asignaciones manuales hechas luego desde el panel admin.
    """
    conn   = sqlite3.connect(RUTA_BD)
    cursor = conn.cursor()
    ...
    # Odontólogos activos (rol 2, en cualquiera de las dos tablas de roles)
    cursor.execute("""
    ...
    """)
    odontologos_activos = [r[0] for r in cursor.fetchall()]

    cursor.execute("SELECT id_odontologo FROM tblodontologo_sala")
    ya_tienen_sala = {r[0] for r in cursor.fetchall()}

    pendientes = [oid for oid in odontologos_activos if oid not in ya_tienen_sala]

    if pendientes:
        # Buscar el siguiente número de "Consultorio N" disponible,
        # para no chocar con nombres ya usados (Consultorio 1, 2, etc.)
        cursor.execute("""
        ...
        for id_odontologo in pendientes:
            ...
            print(f"✅ Odontólogo {id_odontologo} → {nombre_sala} (id_sala {id_sala_nueva}) asignado.")
            siguiente_numero += 1

        conn.commit()
    else:
        print("📁 Todos los odontólogos activos ya tienen consultorio asignado.")

    conn.close()


# ── Ejecutar todas las migraciones en orden al arrancar ──────────────────────
inicializar_base_de_datos()
migrar_precio_base()
migrar_columna_tratamiento()
migrar_odontologo_servicio()
migrar_odontologo_sala()       # ← NUEVA: consultorio propio por odontólogo
...
# =========================================================================
# ─── MOTOR DE FRONTEND CON PROTECCIÓN DE RUTAS
# =========================================================================

@app.route('/web/<carpeta>/<archivo>', methods=['GET'])
def servir_frontend(carpeta, archivo):
    """Sirve los archivos estáticos con guardia de sesión y roles."""
    roles_usuario = set()
    if 'id_usuario' in session:
        roles_usuario = _roles_frescos(session['id_usuario'])

    # GUARDIA 1: ya inició sesión → no puede volver al login
    if carpeta == 'login' and archivo == 'login.html':
        if roles_usuario:
            if   1 in roles_usuario: redireccion = '/web/recepcionista/panel_rec.html'
            ...
            print(f"🔄 Usuario {session['id_usuario']} ya activo. Redirigiendo.")
            return redirect(redireccion)

    # GUARDIA 2: sin sesión → no puede acceder a la agenda
    if carpeta == 'agenda_cliente' and archivo == 'index.html':
        if not roles_usuario:
            return redirect('/web/login/login.html')

    # GUARDIA 3: panel médico → solo admin(1) u odontólogo(2)
    if carpeta == 'odontologo':
        if not roles_usuario or not (roles_usuario & {1, 2}):
            return redirect('/web/login/login.html')

    # GUARDIA 4: panel recepción → solo admin(1) o recepcionista(3)
    if carpeta == 'recepcionista':
        if not roles_usuario or not (roles_usuario & {1, 3}):
            return redirect('/web/login/login.html')
...
# ─── ANTÍDOTO PARA CACHÉ DEL NAVEGADOR ───────────────────────────────────────
