...
# Guardamos todo tu SQL como un texto gigante dentro de Python
sql_script = """
...
CREATE INDEX IF NOT EXISTS `idx_usuario_rol` ON `tblusuario` (`id_rol`);
...
try:
    print("Conectando y creando 'clinident.db'...")
    conn = sqlite3.connect("clinident.db")
    cursor = conn.cursor()
    
    # Ejecuta todo el bloque SQL
    cursor.executescript(sql_script)
    conn.commit()
    print("¡Éxito! Base de datos 'clinident.db' construida con todas sus tablas y datos.")

except sqlite3.Error as e:
    print(f"Hubo un error al generar la base de datos: {e}")
